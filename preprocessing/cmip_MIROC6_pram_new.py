# Purpose: cmip MIROC6 pram new.
# Source: cmip_MIROC6_pram_new.py; see docs/source_coverage.csv for cell provenance.
# Run order and required inputs: docs/workflow.md.
# Run from repository root: python -m preprocessing.cmip_MIROC6_pram_new

# -*- coding: utf-8 -*-
"""
MIROC6 hybrid-sigma (6hrLev) 环境参数诊断（原始数据）——按年输出 + 真·断点续传（time-block part）
适配点（相对 CanESM5）：
1) MIROC6 hybrid 坐标：p = a*p0 + b*ps   （a,b无量纲；p0,ps为Pa）
...
# ---------------- 用户配置（按需修改） ----------------
ROOT = Path(resource_path('cmip6_raw', 'MIROC6'))
OROG_DIR = ROOT / "orog"
OUT_DIR  = Path(resource_path('models', 'MIROC6/envdiag'))
...
# MIROC6 hybrid 压强与高度
# =========================
def compute_pressure_pa_miroc(a_lev: np.ndarray, b_lev: np.ndarray, p0: float, ps_2d: np.ndarray) -> np.ndarray:
    """
    MIROC6 (CMIP6 hybrid-sigma):
      p = a*p0 + b*ps
...
# 核心：单时次诊断（MIROC6）
# =========================
def diag_one_time_miroc(a_lev, b_lev, p0_ref, ps2d, orog2d,
                       T_lev, q_lev, u_lev, v_lev,
                       lat, lon, dx, dy):

    # 1) p 与 H（确保 surface->top：由 p_med 判定反转）
    p3 = compute_pressure_pa_miroc(a_lev, b_lev, float(p0_ref), ps2d).astype(np.float64)
    lev, ny, nx = p3.shape
...
def build_var_timekey_map(exp: str, var: str) -> Dict[Tuple[str, str], Path]:
    patt = f"{var}_6hrLev_MIROC6_{exp}_r1i1p1f1_gn_*.nc"
...
def infer_orog_file(exp: str) -> Path:
    cand = sorted(OROG_DIR.glob(f"orog_fx_MIROC6_{exp}_r1i1p1f1_gn.nc"))
    if not cand:
        raise FileNotFoundError(f"找不到地形文件：orog_fx_MIROC6_{exp}_r1i1p1f1_gn.nc 于 {OROG_DIR}")
    return cand[0]
...
def build_year_tasklist(exp: str,
                        ...
                        year: int) -> List[Tuple[Tuple[str,str], int, np.datetime64]]:
    tasks: List[Tuple[Tuple[str,str], int, np.datetime64]] = []
    y0 = np.datetime64(f"{year:04d}-01-01T00:00:00")
    y1 = np.datetime64(f"{year+1:04d}-01-01T00:00:00")

    for key in keys:
        hus_path = var_maps["hus"][key]
        with xr.open_dataset(hus_path) as ds:
            t = ds["time"].values
        mask = (t >= y0) & (t < y1)
        if not np.any(mask):
            continue
        ...
        tsel = t[idx]
        for ii, tv in zip(idx, tsel):
            tasks.append((key, int(ii), np.datetime64(tv)))
...
def process_experiment(exp: str):
    ...
    for y in years:
        out_nc = OUT_DIR / f"MIROC6_{exp}_envdiag_{y}.nc"
        if out_nc.exists():
        ...
        expected_part_paths = [OUT_DIR / f"MIROC6_{exp}_envdiag_{y}.part{ip:03d}.nc" for ip in range(n_parts)]
        ...
        for ip in range(n_parts):
            ...
            tvals = np.array([tv for (_, _, tv) in chunk], dtype="datetime64[ns]")
        ...
        part_files = sorted(OUT_DIR.glob(f"MIROC6_{exp}_envdiag_{y}.part*.nc"))
