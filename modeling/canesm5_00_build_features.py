# Purpose: Generate CanESM5 historical and SSP585 2-degree feature tables using the manuscript's 15 non-seasonal statistics.
# Source: author-supplied canesm5_feature_reference.py; paths and confirmed feature schema packaged.
# Run order: after the model's envdiag script and before label merging/training.
# Run from repository root: python -m modeling.canesm5_00_build_features
...
# 输出目录：建议分开
OUT_TRAIN_DIR  = resource_path('models', 'CanESM5/feature/ml_train_historical')          # 骨架 + historical features
OUT_FUTURE_DIR = resource_path('models', 'CanESM5/feature/ml_pred_ssp585')   # future decade features
os.makedirs(OUT_TRAIN_DIR, exist_ok=True)
os.makedirs(OUT_FUTURE_DIR, exist_ok=True)

# ROI: [N, W, S, E]  —— 按你最近一直使用的 ROI63
...
# 历史期（这里只用于生成 historical features）
...
def open_envdiag(kind: str, year0: int, year1: int) -> xr.Dataset:
    if kind == "historical":
        ...
        for y in range(year0, year1 + 1):
            f = os.path.join(ENV_DIR, f"CanESM5_ssp585_envdiag_{y}.nc")
            if os.path.exists(f):
                files.append(f)
        if not files:
            raise FileNotFoundError(f"ssp585 {year0}-{year1} 没找到任何文件")
    else:
...
# ---------------------------
# 6) Step-5：正文15项非季节统计特征
# ---------------------------
def _time_quantile(da: xr.DataArray, q: float) -> xr.DataArray:
    """Calculate one time quantile without retaining a scalar quantile column."""
...
def add_15_statistics(feats: xr.Dataset, da: xr.DataArray, prefix: str):
    """Add the 15 non-seasonal statistics stated in the manuscript."""
    mean = da.mean("time", skipna=True)
    std = da.std("time", skipna=True)
    minimum = da.min("time", skipna=True)
    maximum = da.max("time", skipna=True)
    quantiles = {
        name: _time_quantile(da, q)
...
def build_features(ds2: xr.Dataset) -> xr.Dataset:
    feats = xr.Dataset()
    for v in tqdm(list(ds2.data_vars), desc="Build 15 statistics (vars)", unit="var"):
        da = ds2[v]
        prefix = f"{FEATURE_PREFIX}_{v}"
        add_15_statistics(feats, da, prefix)

    expected = len(VARS) * len(STATISTIC_SUFFIXES)
...
def compute_features_to_parquet(kind: str, year0: int, year1: int, lat2, lon2, out_dir: str, df_elev_grid: pd.DataFrame) -> pd.DataFrame:
    """
