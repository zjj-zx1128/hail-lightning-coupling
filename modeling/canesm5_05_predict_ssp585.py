# -*- coding: utf-8 -*-
"""
Generate CanESM5 SSP585 decadal lightning- and hail-frequency predictions.

Inputs are the standardized ``FINAL_XGB`` models/feature lists and the SSP585
feature tables prepared by ``canesm5_01_prepare_training_data.py``. Negative
predicted frequencies are clipped to zero, following the supplied workflow.

Run from the repository root:
    python -m modeling.canesm5_05_predict_ssp585
"""
...
def save_masks(source_df: pd.DataFrame, lat_axis: np.ndarray, lon_axis: np.ndarray):
    ...
    mask_ds = xr.Dataset(
        ...
        attrs={
            ...
            "note": (
                "strict_land was reconstructed from the strict-land training table. "
