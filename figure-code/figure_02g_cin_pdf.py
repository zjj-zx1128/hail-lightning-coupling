...
output_folder = resource_path('figures', 'figure_02g/')
...
for i, f in enumerate(hl_files):
    ...
    cin_values = df["CIN"]
    cin_values = cin_values[(cin_values > 0) & (~cin_values.isna())]
    log_data = np.log10(cin_values)
    
    fixed_bins = np.linspace(0, 3, 26)
...
# 设置图形样式
plt.xlabel("CIN (J/kg)", fontsize=20)
plt.ylabel("Probability Density", fontsize=20)
plt.title("(g) CIN", fontsize=25, fontweight='bold', pad=10)
plt.xlim(0, 3)
plt.ylim(0, 1)
...
# 保存图像
output_path = os.path.join(output_folder, 'figure_02g_cin_pdf.png')
